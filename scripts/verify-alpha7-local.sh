...
usage() {
  cat <<'EOF'
Usage: scripts/verify-alpha7-local.sh [--device] [--clean]

Runs the deterministic build checks for the v0.3.0-alpha.7 candidate.
...
Environment:
  ...
  CONNECTX_VERIFY_OUT              Output directory. Defaults to
                                   build/local-verification-alpha7.
...
OUT_DIR="${CONNECTX_VERIFY_OUT:-$ROOT_DIR/build/local-verification-alpha7}"
...
log "Running guarded release manifest tests"
...
grep -q 'versionName = "0.3.0-alpha.7"' app/build.gradle.kts
test -s docs/architecture/external-tls-evidence.md
test -s docs/releases/v0.3.0-alpha.7.md
test -s docs/roadmap/alpha7-scope.md
grep -Fq '## [0.3.0-alpha.7]' CHANGELOG.md
...
cp "$APK_PATH" "$OUT_DIR/apk/ConnectX-v0.3.0-alpha.7-debug.apk"
rm -rf "$OUT_DIR/native/jniLibs"
cp -R engine/go/build/android/jniLibs "$OUT_DIR/native/jniLibs"
(
  cd "$OUT_DIR"
  sha256sum \
    apk/ConnectX-v0.3.0-alpha.7-debug.apk \
...
ConnectX v0.3.0-alpha.7 local verification passed.
Commit: $COMMIT_SHA
