...
usage() {
  cat <<'EOF'
Usage: scripts/verify-alpha6-local.sh [--device] [--clean]

Runs the deterministic build checks for the v0.3.0-alpha.6 candidate.
...
Environment:
  ...
  CONNECTX_VERIFY_OUT              Output directory. Defaults to
                                   build/local-verification-alpha6.
...
OUT_DIR="${CONNECTX_VERIFY_OUT:-$ROOT_DIR/build/local-verification-alpha6}"
...
grep -q 'versionName = "0.3.0-alpha.6"' app/build.gradle.kts
test -s docs/architecture/external-tls-evidence.md
test -s docs/releases/v0.3.0-alpha.6.md
test -s docs/roadmap/alpha6-scope.md
grep -Fq '## [0.3.0-alpha.6]' CHANGELOG.md
...
cp "$APK_PATH" "$OUT_DIR/apk/ConnectX-v0.3.0-alpha.6-debug.apk"
rm -rf "$OUT_DIR/native/jniLibs"
cp -R engine/go/build/android/jniLibs "$OUT_DIR/native/jniLibs"
(
  cd "$OUT_DIR"
  sha256sum \
    apk/ConnectX-v0.3.0-alpha.6-debug.apk \
...
ConnectX v0.3.0-alpha.6 local verification passed.
Commit: $COMMIT_SHA
