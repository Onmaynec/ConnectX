...
internal object Socks5Protocol {
    ...
    private const val AUTH_NO_AUTHENTICATION = 0x00
    ...
    fun negotiateAuthentication(
        ...
        repeat(methodCount) {
            if (input.readUnsignedByteOrThrow() == AUTH_NO_AUTHENTICATION) {
        ...
        if (!supportsNoAuthentication) {
            throw IOException("SOCKS-клиент не поддерживает локальный режим без авторизации")
        }
    }
