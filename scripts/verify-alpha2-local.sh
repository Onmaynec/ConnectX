...
usage() {
  cat <<'EOF'
Usage: scripts/verify-alpha2-local.sh [--device] [--clean]

Runs the same deterministic build checks used by Android CI for the
v0.3.0-alpha.2 candidate.

  --device  Also run the isolated Android instrumentation gates on one
            already-running Android 35 x86_64 emulator/device.
  --clean   Run a Gradle clean and remove the previous native output first.
  --help    Show this help.
...
Environment:
  ...
  ADB_SERIAL                       Device serial when more than one is visible.
  ...
  CONNECTX_ALLOW_DEVICE_VARIANT=1  Permit a non-Android-35 or non-x86_64
                                   device for supplementary testing. Such a
                                   run does not reproduce the official gate.
EOF
}
...
OUT_DIR="${CONNECTX_VERIFY_OUT:-$ROOT_DIR/build/local-verification}"
...
cp "$APK_PATH" "$OUT_DIR/apk/ConnectX-v0.3.0-alpha.2-debug.apk"
rm -rf "$OUT_DIR/native/jniLibs"
cp -R engine/go/build/android/jniLibs "$OUT_DIR/native/jniLibs"
(
  cd "$OUT_DIR"
  sha256sum \
    apk/ConnectX-v0.3.0-alpha.2-debug.apk \
...
if ((RUN_DEVICE_GATES)); then
  ...
  if [[ "${CONNECTX_ALLOW_DEVICE_VARIANT:-0}" != "1" ]]; then
    [[ "$DEVICE_SDK" == "$EXPECTED_ANDROID_SDK" ]] ||
      fail "Official local reproduction requires Android $EXPECTED_ANDROID_SDK; found $DEVICE_SDK"
    [[ "$DEVICE_ABI" == "$EXPECTED_ANDROID_ABI" ]] ||
      fail "Official local reproduction requires $EXPECTED_ANDROID_ABI; found $DEVICE_ABI"
  else
    echo "WARNING: device variant override is enabled; this is supplementary evidence only." |
...
ConnectX v0.3.0-alpha.2 local verification passed.
Commit: $COMMIT_SHA
