...
usage() {
  cat <<'EOF'
Usage: scripts/verify-alpha3-local.sh [--device] [--clean]

Runs the deterministic build checks for the v0.3.0-alpha.3 candidate.
...
Environment:
  ...
  CONNECTX_VERIFY_OUT              Output directory. Defaults to
                                   build/local-verification-alpha3.
...
OUT_DIR="${CONNECTX_VERIFY_OUT:-$ROOT_DIR/build/local-verification-alpha3}"
...
grep -q 'versionName = "0.3.0-alpha.3"' app/build.gradle.kts
...
cp "$APK_PATH" "$OUT_DIR/apk/ConnectX-v0.3.0-alpha.3-debug.apk"
rm -rf "$OUT_DIR/native/jniLibs"
cp -R engine/go/build/android/jniLibs "$OUT_DIR/native/jniLibs"
(
  cd "$OUT_DIR"
  sha256sum \
    apk/ConnectX-v0.3.0-alpha.3-debug.apk \
...
ConnectX v0.3.0-alpha.3 local verification passed.
Commit: $COMMIT_SHA
